...
genome <- GenomeRetrieval("Saccharomyces cerevisiae")
...
genome |> 
  filter(grepl("wine001", Title)) |> 
  filter(Seq.Length > 10000) -> genome

## Filter sequences based on length
genome |> 
  filter(Seq.Length > 100000)

# Choosing only 100 rows from the data table
genome <- genome |> 
...
genes <- GeneRetrieval("Alcohol dehydrogenase", "Saccharomyces cerevisiae")
